...
class TestProfileSessionIsolation(unittest.TestCase):
    ...
    def test_profile_isolated_by_session(self):
        """不同会话的画像写入不同文件,互不污染"""
        with patch.object(builtins_module, "MEMORY_DIR", self.tmp):
            # 会话 work 写入画像
            work_tool = builtins_module.create_profile_tool("work")
            result = work_tool.invoke({"new_content": "# work 画像\n- 工作:写报告"})
            self.assertIn("记忆档案已成功覆写更新", result)

            # 会话 personal 写入不同画像
            personal_tool = builtins_module.create_profile_tool("personal")
    ...
    def test_profile_diff_logged(self):
        ...
        with patch.object(builtins_module, "MEMORY_DIR", self.tmp):
            with patch.object(builtins_module.audit_logger, "log_event") as mock_log:
                tool = builtins_module.create_profile_tool("work")
    ...
    def test_default_profile_migrated_from_legacy(self):
        ...
        # 旧文件在 MEMORY_DIR 根(user_profile.md),patch 后 MEMORY_DIR=self.tmp
        ...
        with patch.object(builtins_module, "MEMORY_DIR", self.tmp):
            builtins_module.migrate_legacy_profile("local_geek_master")
