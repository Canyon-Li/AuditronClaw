...
**扩展边界**：安全声明（上表与基准数字）均针对内置工具面。外部能力经两条通道接入：SKILL.md 技能层零代码接入，run 命令与手动 shell 走同一校验器；代码级注入（`create_agent_app` 的 `tools` / `extra_tools` 参数）不经命令白名单与路径防护，调用全程落审计，但恶意扩展不做拦截，安全责任由注入方自担。收口方向为统一策略门（外接工具强制过门）。
